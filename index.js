...
const showApiData = (loopData, isSeeMore) => {
    ...
    <div class="border-b-[1px] border-[#11111133] pb-6">
        ...
        <div class="text-[#585858] space-y-3 leading-[26px]">
            <p><span> 1 </span> ${singleData?.features[0] || 'not available'}</p>
            <p><span> 2 </span> ${singleData?.features[1] || 'not available'}</p>
            <p><span> 3 </span> ${singleData?.features[2] || 'not available'}</p>           
        </div>
    </div>
...
const showWithModal = detail => {
    console.log(detail);
    const accuracy = `${detail.accuracy.score}`* 100;
...
<div class="flex flex-col-reverse lg:flex-row gap-4">
        ...
        <div class="flex justify-between">
            <div>
                ...
                <ol type="dotted" class="text-[#585858] leading-[26px]">
                    <li>${detail.features["1"]?.feature_name || 'not available'}</li>
                    <li>${detail.features["2"]?.feature_name || 'not available'}</li>
                    <li>${detail.features["3"]?.feature_name || 'not available'}</li>
                </ol>
            </div>
            <div>
                ...
                <ol class="text-[#585858] leading-[26px]">
                    <li>${detail?.integrations[0] || 'not available'}</li>
                    <li>${detail?.integrations[1] || 'not available'}</li>
                    <li>${detail?.integrations[2] || 'not available'}</li>
                </ol>
            </div>
        </div>
    </div>
    ...
    <div class="border-[1px] border-[#E7E7E7] bg-white p-2 lg:p-[30px] rounded-lg  lg:w-[50vw]">
        <div class="flex justify-center relative">
            <img class="h-[25vh] lg:h-auto rounded-lg" src="${detail?.image_link[0] || detail.image_link[1]}" alt="modal-image">
            <h1 class="text-white bg-button-color px-[15px] py-[5px] font-semibold leading-[19px] rounded-lg absolute right-1">${accuracy} <span> % accuracy </span> </h1>
        </div>
